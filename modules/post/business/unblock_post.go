...
func (biz *postBusiness) UnblockPost(ctx context.Context, postId *string, user *common.Requester) (*entity.Post, error) {
	...
	if !post.IsBlocked {
		return nil, common.NewCustomBadRequestError("Post not blocked")
	}

	updatedPost, err := biz.postRepo.Update(
		ctx,
		map[string]interface{}{"id": *postId},
		map[string]interface{}{"is_blocked": false},
	)
