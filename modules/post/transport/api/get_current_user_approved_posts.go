...
func (hdl *postHandler) GetCurrentUserApprovedPosts(appCtx appctx.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		posts, paginationInfo, err := hdl.business.GetCurrentUserApprovedPosts(c.Request.Context(), pagination, &requester)
		if err != nil {
			panic(err)
		}
		postCount := len(posts)
